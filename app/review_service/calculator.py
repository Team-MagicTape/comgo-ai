...
# 프롬프트를 수정하여, JSON 포맷 지침을 포함시킵니다.
review_prompt = ChatPromptTemplate.from_template(
    template="""
    당신은 인지 과학 및 에빙하우스의 망각 곡선 이론에 기반한 SRS(간헐적 반복 학습) 전문가입니다.
    사용자의 퀴즈 리뷰 데이터를 바탕으로, 최적의 다음 복습 주기를 '일(day)' 단위로 계산해야 합니다.

    # 입력 데이터:
    - 직전 학습 결과: {is_correct} (true=정답)
    - 현재 복습 주기 (일): {current_interval} (첫 학습일 경우 0)

    # 계산 규칙:
    1.  직전 학습 결과가 '정답'(true)이었으므로, 현재 복습 주기를 1.6배에서 2.2배 사이로 늘립니다. 현재 주기가 길수록 증가율을 높게 적용할 수 있습니다.
...
async def calculate_next_interval(is_correct: bool, current_interval: int) -> int:
    ...
    # 오답인 경우, AI를 호출하지 않고 즉시 1을 반환합니다. (Guard Clause)
    if not is_correct:
        return 1

    try:
        # 이제 이 코드는 is_correct가 항상 true인 경우에만 실행됩니다.
        ...
        # 정답인 경우의 fallback 로직: 현재 주기에 하루를 더합니다.
